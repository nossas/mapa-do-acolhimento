...
const generateVariables = (matches: Match[]) =>
  matches.map((_, index) => generateVariablesIndex(index)).flat();

const generateObjects = (matches: Match[]) =>
  `[${matches
    .map((_, index) => `{${generateObjectsIndex(index)}}`)
    .join(",")}]`;

const createQuery = (matches: Match[]) => `mutation (${generateVariables(
  matches
)}) {
