...
const rMautic = async (
  ...
  const token = Buffer.from(`${mautic_username}:${mautic_password}`).toString(
    "base64"
  );
  return axios({
    method: "get",
...
(async () => {
  ...
  const forms = await rMautic(
    "/forms?publishedOnly=1",
    mautic_username,
    mautic_password
  );
  ...
  const submissions = await rMautic(
    `/forms/${response2.selectedForm}/submissions?limit=3`,
    mautic_username,
    mautic_password
  );
